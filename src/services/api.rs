...
use crate::models::freebox::lan::{GetLanResponse, LanConfigBody, ListLanCountResponse, ListLanResponse, UpdateLanResponse, WakeOnLanBody, WakeOnLanResponse};
...
pub trait FreeboxOSApiCalls<T: HttpClient> {
    ...
    async fn login( &self, client: &T, session_token: Option<String>) -> Result<AuthLoginResponse, T::Error>;
    async fn logout(&self, client: &T, session_token: &String, ) -> Result<AuthLogoutResponse, T::Error>;

    async fn get_system_info(&self, client: &T, session_token: &String) -> Result<GetSystemInfoRequest, T::Error>;
    async fn reboot(&self, client: &T, session_token: &String) -> Result<RebootRequest, T::Error>;
    async fn shutdown(&self, client: &T, session_token: &String) -> Result<ShutdownRequest, T::Error>;

    async fn list_lan_interfaces(&self, client: &T, session_token: &String) -> Result<ListLanCountResponse, T::Error>;
    async fn list_devices(&self, client: &T, session_token: &String, interface: String) -> Result<ListLanResponse, T::Error>;
    async fn get_device(&self, client: &T, session_token: &String, interface: String, device_id: String) -> Result<GetLanResponse, T::Error>;
    async fn update_device(&self, client: &T, session_token: &String, interface: String, device_id: String, body: LanConfigBody) -> Result<UpdateLanResponse, T::Error>;
    async fn woa_device(&self, client: &T, session_token: &String, interface: String, body: WakeOnLanBody) -> Result<WakeOnLanResponse, T::Error>;

    async fn get_update_status(&self, client: &T, session_token: &String) -> Result<GetUpdateStatusResponse, T::Error>;
}
